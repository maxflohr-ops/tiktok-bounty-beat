...
const isSpotify = (u: string) =>
  /(^|\/\/)([a-z]+\.)?spotify\.com\//i.test(u) || u.startsWith("spotify:");
    ...
    const slug = path
      ...
      .filter(
        (s) =>
          ...
          !/^\d+$/.test(s) &&
          !/^[a-z]{2}$/i.test(s) &&
          !/^(track|album|song|artist|playlist|music)$/i.test(s),
      )
...
export function soundLinks(b: SoundLinkSource): SoundLink[] {
  ...
  if (src && isSpotify(src))
    links.push({ platform: "spotify", label: "spotify", url: src, exact: true });
  else if (src && isApple(src))
    links.push({ platform: "apple", label: "apple music", url: src, exact: true });
  ...
  if (tiktok) {
    ...
    links.push({
      platform: "tiktok",
      label: "tiktok sound",
      url: tiktokSoundSearchUrl(query),
      exact: false,
    });
  }
  ...
  if (query) {
    links.push({
      ...
      url: youtubeShortsSoundUrl(query),
      exact: false,
    });
  }
