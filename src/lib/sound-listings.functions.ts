  ...
  .superRefine((d, ctx) => {
    if (d.listing_type === "stream") {
      if (!d.stream_url) {
        ...
        ctx.addIssue({
          code: "custom",
          message: "Use a Twitch, YouTube, or Kick link",
          path: ["stream_url"],
        });
      }
    }
    if ((d.listing_type === "keynote" || d.listing_type === "podcast") && !d.stream_url) {
      ctx.addIssue({
        code: "custom",
        message: "This listing needs a footage link",
        path: ["stream_url"],
      });
    }
  });
  ...
  const lineItems = [
    {
      price_data: {
        ...
        product_data: {
          name: `${
            { sound: "Sound", stream: "Stream", keynote: "Keynote", podcast: "Podcast" }[
              data.listing_type
            ]
        ...
        product_data: {
          name:
            data.featured_tier === "featured_plus"
              ? "Featured+ placement — first month"
              : "Featured placement — first month",
          description:
            data.featured_tier === "featured_plus"
...
export const listMySoundListings = createServerFn({ method: "GET" })
  ...
  .handler(async ({ context }) => {
    const { data, error } = await context.supabase
      .from("sound_listings")
      .select(
        "id,listing_type,artist_name,song_title,status,listed_at,expires_at,amount_cents,currency,created_at,tiktok_sound_url,spotify_url,stream_url,stream_at",
      )
