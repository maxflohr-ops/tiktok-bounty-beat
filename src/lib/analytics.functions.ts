...
export const getAdminAnalytics = createServerFn({ method: "GET" })
  ...
  .handler(async ({ context }) => {
    ...
    const [profilesQ, subsQ, bountiesQ, listingsQ, disputesQ] = await Promise.all([
      supabaseAdmin
        .from("profiles")
        .select("id,display_name,tiktok_handle,points,created_at,wallet_address"),
      supabaseAdmin
        .from("submissions")
        .select(
      ...
      supabaseAdmin
        .from("bounties")
        .select(
          "id,contract_no,title,status,funded_cash_cents,reward_cash_cents,payout_type,deadline,created_at",
        ),
    ...
    >();
    for (const s of subs) {
      const e = byEditor.get(s.editor_id) ?? {
        claims: 0,
        delivered: 0,
        approved: 0,
        views: 0,
    ...
    const bountyTitle = new Map(
      bounties.map((b) => [b.id, `#${String(b.contract_no).padStart(3, "0")} ${b.title}`]),
    );
    const recent = subs
      .map((s) => {
        ...
        return {
          ...
          bounty: bountyTitle.get(s.bounty_id) ?? "contract",
          status: s.status,
        };
      })
      .filter((r) => r.at)
    ...
    return {
      totals: {
        ...
        listing_revenue_cents: sum(
          listings.filter((l) => l.status !== "pending_payment").map((l) => l.amount_cents),
        ),
